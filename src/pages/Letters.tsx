...
import { submitLetter } from "@/api/forms";
...
const Letters = () => {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    const res = await submitLetter(form);
    ...
    if (res.success) {
      ...
      toast.error(res.error || "Failed to submit letter. Please try again.");
    }
  };
    ...
    <main className="bg-background text-foreground min-h-screen">
      ...
      <section className="pt-28 sm:pt-32 md:pt-40 pb-12 md:pb-20 border-b border-foreground/10">
        <div className="max-w-[900px] mx-auto px-4 sm:px-6 md:px-10">
          ...
          <p className="mt-6 max-w-xl text-foreground/75 text-base sm:text-lg">
            {t("letters.dek")}
          </p>
        </div>
      </section>
        ...
        <div className="max-w-[900px] mx-auto px-4 sm:px-6 md:px-10 grid gap-10">
          {LETTERS.map((l) => (
            <article key={l.slug} className="border-l-2 border-accent pl-6">
              <div className="text-[11px] tracking-[0.22em] uppercase text-foreground/55">
                {l.date} · {l.city}, {l.country}
              </div>
              <h2 className="font-serif text-2xl md:text-3xl mt-2">
                {l.subject}
              </h2>
              <p className="mt-4 text-foreground/85 leading-relaxed font-serif text-lg">
                "{l.body}"
              </p>
      ...
      <section className="py-16 md:py-24 bg-secondary/40 border-t border-foreground/10">
        <div className="max-w-[900px] mx-auto px-4 sm:px-6 md:px-10">
          ...
          <p className="mt-4 text-foreground/70 max-w-xl">
            A response, a dispatch from your city, a quiet provocation. We read
            everything.
          </p>
          <form
            onSubmit={submit}
          ...
          >
            ...
            <button
              ...
              className="md:col-span-2 mt-2 inline-flex items-center justify-center bg-foreground text-background px-8 py-4 text-[12px] tracking-[0.22em] uppercase hover:bg-accent hover:text-foreground transition-colors w-full sm:w-fit disabled:opacity-50 disabled:cursor-not-allowed"
            >
